...
import numpy as np
import model.configs_RegNet as configs
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        if skip.shape[3] != x.shape[3]:
            skip = nnf.interpolate(skip, size=x.shape[2:], mode='bilinear', align_corners=False)
        # print(f"Shape after self.up: {x.shape}")
...
class transformationHead(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, upsampling=1, num_lev=5):
        ...
        # A simple convolution layer for qauternions.
        self.conv2d_quat = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, 
                                padding=kernel_size // 2)
        # Initialize weights and biases (you can also use nn.init if preferred)
        self.conv2d_quat.weight = nn.Parameter(torch.randn_like(self.conv2d_quat.weight) * 1e-5)
        self.conv2d_quat.bias = nn.Parameter(torch.zeros_like(self.conv2d_quat.bias))
        
        # A simple convolution layer for translations.
        self.conv2d_trans = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, 
                                padding=kernel_size // 2)
        # Initialize weights and biases (you can also use nn.init if preferred)
        self.conv2d_trans.weight = nn.Parameter(torch.randn_like(self.conv2d_trans.weight) * 1e-5)
        self.conv2d_trans.bias = nn.Parameter(torch.zeros_like(self.conv2d_trans.bias))
        
        # A simple convolution layer for scale.
        self.conv2d_scale = nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, 
                                padding=kernel_size // 2)
        # Initialize weights and biases (you can also use nn.init if preferred)
        self.conv2d_scale.weight = nn.Parameter(torch.randn_like(self.conv2d_scale.weight) * 1e-5)
        self.conv2d_scale.bias = nn.Parameter(torch.zeros_like(self.conv2d_scale.bias))
        ...
        self.class_quat_pred = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=4)
        self.class_trans_pred = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=3)
        self.class_scale_pred = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=1)
        
        self.class_quat = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=4)
        self.class_trans = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=3)
        self.class_scale = ClassScore(in_channels, hidden_dim=2048, dropout_rate=0.5, num_classes=1)
    ...
    def forward(self, x):
        ...
        alpha = 0.5
        # predict quaternions, translations, and scaling based on convolutions
        mean_quat_x = self.conv2d_quat(x)
        if mean_quat_x.requires_grad:
            mean_quat_x.register_hook(lambda grad: grad * alpha)
        mean_quat_x = self.class_quat_pred(mean_quat_x)
            
        mean_trans_x = self.conv2d_trans(x)
        if mean_trans_x.requires_grad:
            mean_trans_x.register_hook(lambda grad: grad * alpha)
        mean_trans_x = self.class_trans_pred(mean_trans_x)
        
        mean_scale_x = self.conv2d_scale(x)
        if mean_scale_x.requires_grad:
            mean_scale_x.register_hook(lambda grad: grad * alpha)
        mean_scale_x = self.class_scale_pred(mean_scale_x) 
        
        # compute quaternions, translations, and scaling at different pyramid levels during training.
        ...
        if self.training:
            ...
            class_quat_x = [self.class_quat(level) for level in pyramid_x]
            class_trans_x = [self.class_trans(level) for level in pyramid_x]
            class_scale_x = [self.class_scale(level) for level in pyramid_x]
            
            # Stack the scores into a tensor of shape: [num_levels, batch_size, 1]
            stacked_quat_x = torch.stack(class_quat_x, dim=0)
            stacked_trans_x = torch.stack(class_trans_x, dim=0)
            stacked_scale_x = torch.stack(class_scale_x, dim=0)

            # Compute the mean over the combined pyramid dimension
            mean_quat_scores = torch.mean(stacked_quat_x, dim=0)  # Shape: [batch_size, 1]
            mean_trans_scores = torch.mean(stacked_trans_x, dim=0)  # Shape: [batch_size, 1]
            mean_scale_scores = torch.mean(stacked_scale_x, dim=0)  # Shape: [batch_size, 1]
        
            # Optionally, if you need probabilities instead of logits, apply sigmoid:
            # prob_scores = torch.sigmoid(mean_scores)
        
            return mean_quat_x, mean_trans_x, mean_scale_x, mean_quat_scores, mean_trans_scores, mean_scale_scores
        else:
            mean_quat_x = self.conv2d_quat(x)
            mean_quat_x = self.class_quat_pred(mean_quat_x)
            
            mean_trans_x = self.conv2d_quat(x)
            mean_trans_x = self.class_quat_pred(mean_trans_x)
            
            mean_scale_x = self.conv2d_quat(x)
            mean_scale_x = self.class_quat_pred(mean_scale_x)
            
            return mean_quat_x, mean_trans_x, mean_scale_x, None, None, None
...
class RegNet(nn.Module):
    def __init__(self, config):
        ...
        self.c2 = Conv2dReLU(1, config.reg_head_chan, 3, 1, use_batchnorm=False)
        ...
        self.fc2 = nn.Linear(128, 8)  # 8 output parameters
    ...
    def forward(self, x, disp=None,  val=True, mc_dropout=False, test=False):
        ...
        # Get dynamic feature map size
        batch_size, channels, H, W = source1.shape

        # Initialize fc1 dynamically if not already defined
        if self.fc1 is None:
            self.fc1 = nn.Linear(channels * H * W, 128).to(source1.device)

        # Flatten feature map
        source = torch.flatten(source1, start_dim=1)

        # Fully Connected Layers for Regression
        source = nnf.relu(self.fc1(source))
        source = self.fc2(source)

        return source  # Output shape: [batch_size, 8]
    
    
CONFIGS = {
    'RegNet': configs.get_2DRegNet_config(),
}
